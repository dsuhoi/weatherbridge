"""Hierarchical DC-AE 1.5-style spatial compression — v3 (capacious + unfrozen).

Differences vs v2:
  * Inner cascade is CAPACIOUS: 4 ResBlks + ChannelFFN + 4-head SelfAttention2D
    in z₂ (23×46 tokens, cheap), latent_ch_phase2 default 128 (was 96).
  * Phase 1 weights UNFROZEN by default (`--unfreeze_phase1`, on by default).
  * Split-LR optimizer: P1 params at `lr * lr_ratio_p1` (default 0.1), inner
    cascade + adapters at full `lr` — pretrained P1 stays close to its init.
  * latent_loss + SH loss both on by default.

Phase 2 v3 (~10 ep, z₂ (128, 23, 46) ≈ 47× compression):
  inner_down: 4×ResBlk(48) → PixelUnshuffle → 1×1 to 128 → 2×ResBlk → SelfAttn2D → ChannelFFN → ResBlk
  inner_up:   ResBlk(128) → ChannelFFN → SelfAttn2D → 2×ResBlk → 1×1 → PixelShuffle → 4×ResBlk(48)
"""
...
class HierCompressAEv2(pl.LightningModule):
    ...
    def forward(self, x, tisr, static, return_z=False):
        x_in = torch.cat([x, tisr], dim=1)          # (B, 25, 360, 720)
        x_p = self._pad(x_in)                        # (B, 25, 368, 736)
        z1 = self.ae.encoder(x_p)                    # (B, 48, 46, 92)

        if self.phase == 2:
            z2 = self.inner_down(z1)                 # (B, 96, 23, 46)
            ...
            z1_rec = self.inner_up(z2)               # (B, 48, 46, 92)
        else:
            z1_rec = z1
        ...
        # latent_loss is computed against z1 (pre-adapter) so this injection
        # doesn't interfere with the supervision.
        ...
        d_out = self.ae.decoder(z1_for_decoder)      # (B, 25, 368, 736)
        d_out = self._unpad(d_out, target_hw=(360, 720))
        x_rec = d_out[:, :24]                         # slice meteo channels
        if return_z:
