#!/usr/bin/env python3
"""Plot full-year 2020 normalised RMSE from the canonical journal evaluator."""
...
OUT_PDF = ROOT / "paper" / "images" / "fig1_rmse_per_tau.pdf"
TAUS = [1, 2, 3, 4, 5]
...
def _curve(payload: dict, method: str) -> np.ndarray:
    ...
    return np.asarray(
        [
            np.mean(
                [
                    float(payload["per_tau"][str(tau)][method][f"rmse_norm_{channel}"])
...
def main() -> None:
    ...
    ax.set_title("Full-year 2020 interpolation error")
