...
SCREEN_COMPLETE="$LOG_ROOT/under10m_flow_search.complete"
FLOW_CHECKPOINT="$LOG_ROOT/exp_flow_pp3_135_14m_6h_s202707_protocol_v2/epoch=3-step=6568.ckpt"
QUEUE_LOG="$LOG_ROOT/under10m_flow_eval.queue.log"
TERMINAL_MARKER="$LOG_ROOT/under10m_flow_eval.terminal"
COMPLETE_MARKER="$LOG_ROOT/under10m_flow_eval.complete"

mkdir -p "$LOG_ROOT" "$CLIMATOLOGY_CACHE"
exec 9>"$LOG_ROOT/under10m_flow_eval.queue.lock"
if ! flock -n 9; then
  echo "[$(date -Is)] under-10M evaluation already active" \
    | tee -a "$QUEUE_LOG"
  exit 0
fi
...
while [[ ! -e "$SCREEN_COMPLETE" || ! -s "$FINAL_REPORT" ]]; do
  echo "[$(date -Is)] wait under-10M selection" \
    | tee -a "$QUEUE_LOG"
  sleep "$SLEEP_SEC"
done
...
' "$FINAL_REPORT")"
if [[ -z "$selected" ]]; then
  echo "[$(date -Is)] no under-10M model beat Flow at four epochs" \
    | tee -a "$QUEUE_LOG"
...
case "$selected" in
  flow_compact_vp3_l)
    winner_checkpoint="$LOG_ROOT/exp_flow_compact_vp3_l_hf_135_9m_6h_s202707_protocol_v1/epoch=3-step=6568.ckpt"
    ;;
  flow_compact_hermite_l)
    winner_checkpoint="$LOG_ROOT/exp_flow_compact_hermite_l_hf_135_9m_6h_s202707_protocol_v1/epoch=3-step=6568.ckpt"
    ;;
  *)
...
while [[ -z "$gpu" ]]; do
  for candidate in $GPU_CANDIDATES; do
    exec {candidate_fd}>"$LOG_ROOT/.under10m_flow_gpu${candidate}.lock"
...
for year in 2020 2021; do
  out_dir="metrics/under10m_flow_screen_6h_${year}"
  mkdir -p "$out_dir"
  need_field_eval=0
  for artifact_spec in \
    "under10m_winner:$winner_checkpoint" \
  ...
  if [[ "$need_field_eval" -eq 1 ]]; then
    "$PYTHON_BIN" -u tools/eval/batch_eval_12h_memmap.py \
      ...
      --models "under10m_winner:$winner_checkpoint:,flow_4ep_ref:$FLOW_CHECKPOINT:" \
      --out-dir "$out_dir" \
      --paper-tag "under10m_flow_screen_6h_${year}" \
      --batch-size 2 \
      --num-workers 2 \
      --samples-per-date 4 \
      --full-year \
  ...
  for name in under10m_winner flow_4ep_ref; do
    ...
    "$PYTHON_BIN" -u tools/eval/sh_energy_spectra_12h.py \
      ...
      --out-dir "metrics/under10m_flow_spectra_6h_${year}" \
      --taus 1,2,3,4,5 \
      --channels all \
      --lmax 359 \
      --keep-n-channels 24 \
  ...
  "$PYTHON_BIN" tools/eval/paired_block_bootstrap.py \
    --left "under10m_winner:$out_dir/window_metrics/under10m_winner.npz" \
  ...
  for tau in 2 4; do
    "$PYTHON_BIN" tools/eval/spectral_block_bootstrap.py \
      --left "under10m_winner:metrics/under10m_flow_spectra_6h_${year}/under10m_winner_tau${tau}.npz" \
      --right "flow_4ep_ref:metrics/under10m_flow_spectra_6h_${year}/flow_4ep_ref_tau${tau}.npz" \
      --channels all \
      --block-days 7 \
      --draws 5000 \
      --seed 2027 \
      --out-json "metrics/under10m_flow_spectra_6h_${year}/paired_tau${tau}.json" \
      2>&1 | tee -a "$QUEUE_LOG"
  done
done
...
touch "$COMPLETE_MARKER"
trap - ERR
echo "[$(date -Is)] under-10M evaluation complete" \
  | tee -a "$QUEUE_LOG"
