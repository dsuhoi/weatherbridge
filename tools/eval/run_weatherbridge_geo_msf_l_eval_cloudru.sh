...
TRAIN_COMPLETE="$LOG_ROOT/exp_weatherbridge_geo_msf_l_9m_6h_s202707_v2_polarstable_bs8.complete"
MSF_ROOT="$LOG_ROOT/exp_weatherbridge_geo_msf_l_9m_6h_s202707_v2_polarstable_bs8"
FLOW_CHECKPOINT="$LOG_ROOT/exp_flow_pp3_135_14m_6h_s202707_protocol_v2/epoch=3-step=6568.ckpt"
ECONOMY_OUT="metrics/weatherbridge_geo_msf_l_8dpm_6h_2020"
FULL_OUT="metrics/weatherbridge_geo_msf_l_full_6h_2020"
REPORT_ROOT="$METRICS_ROOT/weatherbridge_geo_msf_l_6h"
ECONOMY_REPORT="$REPORT_ROOT/economy_strict_120cell.json"
FULL_REPORT="$REPORT_ROOT/full_year_2020_strict_120cell.json"
QUEUE_LOG="$LOG_ROOT/weatherbridge_geo_msf_l_eval.queue.log"
COMPLETE_MARKER="$LOG_ROOT/weatherbridge_geo_msf_l_eval.complete"
TERMINAL_MARKER="$LOG_ROOT/weatherbridge_geo_msf_l_eval.terminal"

declare -A CHECKPOINTS=(
  [weatherbridge_geo_msf_l_2ep]="$MSF_ROOT/epoch=1-step=3284.ckpt"
  [weatherbridge_geo_msf_l_4ep]="$MSF_ROOT/epoch=3-step=6568.ckpt"
)
...
run_eval() {
  ...
  "$PYTHON_BIN" -u tools/eval/batch_eval_12h_memmap.py \
    ...
    --models "$models" \
    --out-dir "$out_dir" \
    --paper-tag weatherbridge_geo_msf_l_6h_2020 \
    --batch-size 2 \
    --num-workers 2 \
    --samples-per-date 4 \
    --max-tau-hours 6 \
...
exec 9>"$LOG_ROOT/weatherbridge_geo_msf_l_eval.queue.lock"
if ! flock -n 9; then
  echo "[$(date -Is)] WeatherBridge-GeoMSF-L evaluation already active" \
    | tee -a "$QUEUE_LOG"
  exit 0
fi
...
while [[ ! -e "$TRAIN_COMPLETE" ]]; do
  echo "[$(date -Is)] wait WeatherBridge-GeoMSF-L training" | tee -a "$QUEUE_LOG"
  sleep "$SLEEP_SEC"
done
...
run_eval \
  "$ECONOMY_OUT" \
  "weatherbridge_geo_msf_l_2ep:${CHECKPOINTS[weatherbridge_geo_msf_l_2ep]}:,weatherbridge_geo_msf_l_4ep:${CHECKPOINTS[weatherbridge_geo_msf_l_4ep]}:,flow_4ep_ref:$FLOW_CHECKPOINT:" \
...
select_strict \
  "$ECONOMY_REPORT" \
  --candidate "weatherbridge_geo_msf_l_2ep=$ECONOMY_OUT/weatherbridge_geo_msf_l_2ep.json" \
  --candidate "weatherbridge_geo_msf_l_4ep=$ECONOMY_OUT/weatherbridge_geo_msf_l_4ep.json"

selected="$(selected_from "$ECONOMY_REPORT")"
if [[ -z "$selected" ]]; then
  touch "$COMPLETE_MARKER"
  trap - ERR
  echo "[$(date -Is)] no strict 120-cell GeoMSF-L winner" | tee -a "$QUEUE_LOG"
  exit 0
fi
...
touch "$COMPLETE_MARKER"
trap - ERR
echo "[$(date -Is)] WeatherBridge-GeoMSF-L evaluation complete" \
  | tee -a "$QUEUE_LOG"
