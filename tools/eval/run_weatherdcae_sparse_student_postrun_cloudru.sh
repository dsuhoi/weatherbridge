#!/usr/bin/env bash
# Full post-selection test pipeline for the sparse-distilled WeatherDCAE student.
set -euo pipefail
export PYTHONDONTWRITEBYTECODE=1

PY=${PY:-/home/jovyan/.mlspace/envs/ai_scientist/bin/python}
SOURCE=${SOURCE:-/home/jovyan/shares/SR006.nfs2/dsuhoi/weatherdcae_sparse_postrun_v18_source}
LOG_ROOT=${LOG_ROOT:-/home/jovyan/shares/SR006.nfs2/dsuhoi/capmatched_logs}
OUT_ROOT=${OUT_ROOT:-/home/jovyan/shares/SR006.nfs2/dsuhoi/distilled_weatherdcae_sparse_postrun_v18}
SCREEN_ROOT=${SCREEN_ROOT:-/home/jovyan/shares/SR006.nfs2/dsuhoi/distill_weatherdcae_sparse_v17}
...
MATCHED_CONTROL=$LOG_ROOT/exp_weatherdcae_14m_6h_sparse_gt_control_s202712_v17_bs4/last.ckpt
STUDENT=$LOG_ROOT/exp_weatherdcae_14m_6h_sparse_flow_student_s202712_v17_bs4/last.ckpt
STARTING_DCAE=$LOG_ROOT/exp_weatherdcae_14m_6h_direct_q_gt_control_s202710_v12_bs4/last.ckpt
FLOW_TEACHER=/home/jovyan/shares/SR006.nfs2/dsuhoi/distill_shared_merge_v15/checkpoints/merge_a050.ckpt
ROUTE=$SCREEN_ROOT/direct_route_2020.json
...
if ! flock -n 8; then
  echo "[$(date -Is)] WeatherDCAE sparse-student postrun already active"
  exit 0
fi

(cd "$SOURCE" && sha256sum --quiet -c SOURCE_SHA256SUMS)
test -s "$SOURCE/examples/_bare_loader.py"
export PYTHONPATH="$EXTRA_PYTHONPATH:$SOURCE:${PYTHONPATH:-}"

while [[ ! -e "$SCREEN_ROOT/state/strict.pass" ]]; do
  if [[ -e "$SCREEN_ROOT/state/strict.failed" ]]; then
    echo "[$(date -Is)] WeatherDCAE sparse distillation failed strict screen"
    touch "$STATE/selection.failed"
    exit 2
  fi
  echo "[$(date -Is)] wait WeatherDCAE sparse distillation strict screen"
  sleep "$POLL_SECONDS"
done
...
if [[ "$variant" != weatherdcae_flow_student ]]; then
  echo "unexpected WeatherDCAE sparse student: $variant" >&2
  exit 2
fi
for checkpoint in "$STUDENT" "$MATCHED_CONTROL" "$STARTING_DCAE" \
  "$FLOW_TEACHER" "$ROUTE"; do
  test -s "$checkpoint"
done

$PY - "$STUDENT" "$MATCHED_CONTROL" "$STARTING_DCAE" "$FLOW_TEACHER" \
  "$ROUTE" "$variant" <<'PY'
import hashlib
import sys
...
control_hparams = matched_control.get("hyper_parameters", {})
protocol = hparams.get("training_protocol", {})
control_protocol = control_hparams.get("training_protocol", {})
distill = protocol.get("distillation", {})
control_distill = control_protocol.get("distillation", {})
state = checkpoint.get("state_dict", {})
reference = matched_control.get("state_dict", {})
assert sys.argv[6] == "weatherdcae_flow_student"
assert hparams.get("arch") == control_hparams.get("arch") == "dcae_14m"
assert sum(v.numel() for k, v in state.items() if k.startswith("net.")) == 14365049
assert int(checkpoint.get("global_step", -1)) >= 410
assert int(matched_control.get("global_step", -1)) >= 410
for item in (protocol, control_protocol):
    assert item.get("train_years") == [2014, 2015, 2016, 2017, 2018, 2019]
    assert item.get("val_years") == [2020]
    assert item.get("train_tau_hours") == [1, 3, 5]
    assert item.get("eval_tau_hours") == [1, 2, 3, 4, 5]
    assert item.get("global_effective_batch_size") == 16
    assert item.get("trainable_scope") == "all"
    assert item.get("optimizer_weight_decay") == 1.0e-4
base_sha = hashlib.sha256(Path(sys.argv[3]).read_bytes()).hexdigest()
teacher_sha = hashlib.sha256(Path(sys.argv[4]).read_bytes()).hexdigest()
route_sha = hashlib.sha256(Path(sys.argv[5]).read_bytes()).hexdigest()
assert hparams["initialization_lineage"]["checkpoint_sha256"] == base_sha
assert control_hparams["initialization_lineage"]["checkpoint_sha256"] == base_sha
assert control_distill.get("enabled") is False
assert distill.get("enabled") is True
assert distill.get("truth_primary") is True
direct = distill["direct"]
assert direct["blend"] == 0.02
assert direct["target"] == "routed_truth_teacher_convex_response"
assert direct["route"]["sha256"] == route_sha
assert direct["teacher_lineage"]["checkpoint_sha256"] == teacher_sha
assert distill["schedule"] == "cosine_decay"
assert distill["decay_end_fraction"] == 0.75
assert distill["every_n_steps"] == 4
assert distill["interval_corrected"] is True
assert not any("teacher" in key or "distill" in key for key in state)
assert state.keys() == reference.keys()
assert all(state[key].shape == reference[key].shape for key in state)
assert any(not torch.equal(state[key], reference[key]) for key in state if key.startswith("net."))
print({"variant": sys.argv[6], "global_step": checkpoint["global_step"]})
PY
...
(cd "$SOURCE" && "$PY" tools/eval/freeze_distilled_student_selection.py \
  ...
  --model "starting_dcae:$STARTING_DCAE" \
  --model "flow_teacher:$FLOW_TEACHER" \
  --output "$FREEZE")
if [[ "$PRECHECK_ONLY" -eq 1 ]]; then
  touch "$STATE/precheck.pass"
  echo "[$(date -Is)] WeatherDCAE sparse-student precheck complete"
  exit 0
fi
...
MODELS="distilled_student:$STUDENT,matched_control:$MATCHED_CONTROL,starting_dcae:$STARTING_DCAE,flow_teacher:$FLOW_TEACHER"
run_year() {
  local gpu="$1" year="$2"
  (
  ...
  for spec in \
    ...
    "starting_dcae:$STARTING_DCAE" "flow_teacher:$FLOW_TEACHER"; do
    name=${spec%%:*}
    checkpoint=${spec#*:}
  ...
  (cd "$SOURCE" && "$PY" tools/eval/paired_block_bootstrap.py \
    ...
    --right "starting_dcae:$WINDOW/starting_dcae.npz" \
  ...
  for tau in 1 2 3 4 5; do
    (cd "$SOURCE" && "$PY" tools/eval/spectral_block_bootstrap.py \
      ...
      --right "starting_dcae:$SPECTRA/starting_dcae_tau${tau}.npz" \
...
(cd "$SOURCE" && "$PY" tools/eval/summarize_region_season_generalization.py \
  ...
  --models distilled_student,matched_control,starting_dcae,flow_teacher \
  --block-days 7 --draws 5000 --seed 2027 \
  --output "$OUT_ROOT/region_season_generalization.json")

HRES_OUT=$OUT_ROOT/hres_2021
mkdir -p "$HRES_OUT"
for spec in "distilled_student:$STUDENT:dcae_14m" "matched_control:$MATCHED_CONTROL:dcae_14m"; do
...
required = [
    ...
    root / "hres_2021/distilled_student.json",
    root / "hres_2021/distilled_student.paired.npz",
    root / "hres_2021/matched_control.json",
    root / "hres_2021/matched_control.paired.npz",
    root / "hres_2021/summary.json",
    root / "bare/distilled_student.pt",
    root / "bare/matched_control.pt",
    ...
    root / "downstream/2020/physics_matched_control.json",
    root / "downstream/2020/diurnal_matched_control.json",
    root / "downstream/2021/physics_distilled_student.json",
    root / "downstream/2021/diurnal_distilled_student.json",
    root / "downstream/2021/physics_matched_control.json",
    root / "downstream/2021/diurnal_matched_control.json",
]
