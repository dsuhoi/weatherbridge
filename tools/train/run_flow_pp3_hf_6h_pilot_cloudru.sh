...
arch="flow_pp3"
exp_name="exp_flow_pp3_hf_135_14m_6h_s202707_protocol_v2"
checkpoint="$LOG_ROOT/$exp_name/last.ckpt"
run_log="$LOG_ROOT/$exp_name.log"
queue_log="$LOG_ROOT/$exp_name.queue.log"
assessment="metrics/upr_lite_screen/flow_pp3_hf_pilot.json"
ablation="metrics/upr_lite_screen/flow_pp3_hf_vs_nohf_2ep.json"
...
if ! flock -n 9; then
  echo "[$(date -Is)] Flow-PP3+HF pilot queue already active" \
    | tee -a "$queue_log"
  exit 0
fi
...
while ! checkpoint_ready \
  "$upr_two_epoch" upr_implicit_global_14m 2 3284 0.05; do
  ...
  echo "[$(date -Is)] wait two-epoch UPR reference" | tee -a "$queue_log"
  sleep "$SLEEP_SEC"
done
while ! checkpoint_ready \
...
if checkpoint_ready \
  "$checkpoint" "$arch" 8 13136 0.05 --require-resume-lineage; then
  touch "$screened_marker"
  touch "$terminal_marker"
  echo "[$(date -Is)] reuse complete Flow-PP3+HF checkpoint=$checkpoint" \
    | tee -a "$queue_log"
  exit 0
fi
if [[ -e "$terminal_marker" && -s "$assessment" ]]; then
  promoted="$("$PYTHON_BIN" tools/eval/pilot_assessment_status.py \
    "$assessment" \
    --expected-candidate flow_pp3_hf \
    --expected-reference upr_implicit_global_14m \
    --print-promoted)"
  if [[ "$promoted" -ne 1 ]]; then
    touch "$screened_marker"
    echo "[$(date -Is)] reuse excluded Flow-PP3+HF pilot" \
      | tee -a "$queue_log"
    exit 0
  fi
fi
...
run_stage() {
  ...
  echo "[$(date -Is)] launch Flow-PP3+HF epochs=8 gpu=$gpu" \
    | tee -a "$queue_log"
  (
...
while ! checkpoint_ready "$checkpoint" "$arch" 2 3284 0.05; do
  ...
  train_status=$?
  set -e
  if checkpoint_ready "$checkpoint" "$arch" 2 3284 0.05; then
    echo "[$(date -Is)] complete Flow-PP3+HF two-epoch screen" \
      | tee -a "$queue_log"
    continue
  fi
...
checkpoint_ready "$candidate_two_epoch" "$arch" 2 3284 0.05
checkpoint_ready "$upr_two_epoch" upr_implicit_global_14m 2 3284 0.05
...
"$PYTHON_BIN" tools/eval/assess_flow_spherical_ep_pilot.py \
  ...
  --pilot-name flow_pp3_hf \
  --reference-name upr_implicit_global_14m \
  --output "$assessment" >>"$queue_log" 2>&1
"$PYTHON_BIN" tools/eval/summarize_matched_training_curves.py \
  --left-csv "$candidate_csv" \
  --right-csv "$flow_csv" \
  --left-name flow_pp3_hf \
  --right-name flow_pp3_nohf \
...
echo "[$(date -Is)] promote Flow-PP3+HF to eight epochs" \
  | tee -a "$queue_log"
while ! checkpoint_ready \
  "$checkpoint" "$arch" 8 13136 0.05 --require-resume-lineage; do
  log_start_bytes="$(stat -c %s "$run_log" 2>/dev/null || echo 0)"
  if ! run_stage; then
    if tail -c "+$((log_start_bytes + 1))" "$run_log" | \
      grep -Eqi "out of memory|CUDA error: out of memory"; then
...
touch "$terminal_marker"
flock -u "$lock_fd"
echo "[$(date -Is)] complete Flow-PP3+HF checkpoint=$checkpoint" \
  | tee -a "$queue_log"
