...
arch="amt"
exp_name="exp_weather_amt_l_14m_6h_s202707_protocol_v4"
checkpoint="$LOG_ROOT/$exp_name/last.ckpt"
run_log="$LOG_ROOT/$exp_name.log"
queue_log="$LOG_ROOT/$exp_name.queue.log"
assessment="metrics/upr_lite_screen/weather_amt_l_pilot_v4.json"
polar_dir="metrics/upr_lite_screen/weather_amt_l_polar_2020"
...
if ! flock -n 9; then
  echo "[$(date -Is)] WeatherAMT-L pilot queue already active" \
    | tee -a "$queue_log"
  exit 0
fi
...
(
  ...
  "$PYTHON_BIN" -u tools/eval/region_season_12h_eval.py \
    ...
    --models "weather_amt_l:$pilot_two_epoch_checkpoint,upr_implicit_global_14m:$upr_two_epoch_checkpoint" \
    --out-dir "$polar_dir" \
    --batch-size 4 \
    --num-workers 2 \
    --samples-per-date 2 \
...
"$PYTHON_BIN" tools/eval/assess_flow_spherical_ep_pilot.py \
  ...
  --pilot-name weather_amt_l \
  --reference-name upr_implicit_global_14m \
  --polar-pilot-json "$polar_dir/weather_amt_l.json" \
...
promoted="$("$PYTHON_BIN" tools/eval/pilot_assessment_status.py \
  "$assessment" \
  --expected-candidate weather_amt_l \
