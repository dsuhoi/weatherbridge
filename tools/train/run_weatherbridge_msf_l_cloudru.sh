...
FROZEN_TRAINER="legacy/training_snapshots/train_capacity_matched_6h_7b36d562.py"
FROZEN_TRAINER_SHA256="7b36d5621a746df9c597e473d8ae17b171aef39dcafc6ea6efb805dc1f8b1d1c"
MODEL_SOURCE="weather_time_interp/model/weatherbridge_flow_model.py"
MODEL_SOURCE_SHA256="9a0343bffb5dda37c5eb860ce18a4b8df337ab76d481c1e196d95b08222aeff7"
...
EXP_NAME="${EXP_NAME:-exp_weatherbridge_msf_l_9m_6h_s202707_v2_bs8}"
...
assert hparams["arch"] == "flow_msf_pareto_l"
...
candidate_ready() {
  "$PYTHON_BIN" tools/train/checkpoint_status.py \
    "$CHECKPOINT" \
    --min-epochs 4 \
    --expected-arch flow_msf_pareto_l \
...
acquire_gpu() {
  ...
  while true; do
    for candidate in $GPU_CANDIDATES; do
      local lock_fd
      exec {lock_fd}>"$LOG_ROOT/.weatherbridge_msf_l_gpu${candidate}.lock"
...
if [[ "$PREFLIGHT_ONLY" == 1 ]]; then
  echo "[$(date -Is)] WeatherBridge-MSF-L preflight passed" | tee -a "$QUEUE_LOG"
  trap - ERR
  exit 0
fi
...
if ! flock -n 9; then
  echo "[$(date -Is)] WeatherBridge-MSF-L worker already active" \
    | tee -a "$QUEUE_LOG"
  exit 0
fi
...
touch "$COMPLETE_MARKER"
trap - ERR
echo "[$(date -Is)] WeatherBridge-MSF-L training complete" \
  | tee -a "$QUEUE_LOG"
