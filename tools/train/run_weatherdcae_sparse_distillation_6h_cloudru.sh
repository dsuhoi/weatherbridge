#!/usr/bin/env bash
# Matched sparse-response distillation of WeatherDCAE from the Flow champion.
set -euo pipefail
export PYTHONDONTWRITEBYTECODE=1

PY=${PY:-/home/jovyan/.mlspace/envs/ai_scientist/bin/python}
SOURCE=${SOURCE:-/home/jovyan/shares/SR006.nfs2/dsuhoi/weatherdcae_sparse_distill_v17_source}
LOG_ROOT=${LOG_ROOT:-/home/jovyan/shares/SR006.nfs2/dsuhoi/capmatched_logs}
OUT_ROOT=${OUT_ROOT:-/home/jovyan/shares/SR006.nfs2/dsuhoi/distill_weatherdcae_sparse_v17}
...
BASE=$LOG_ROOT/exp_weatherdcae_14m_6h_direct_q_gt_control_s202710_v12_bs4/last.ckpt
TEACHER=/home/jovyan/shares/SR006.nfs2/dsuhoi/distill_shared_merge_v15/checkpoints/merge_a050.ckpt
BASE_EVAL=/home/jovyan/shares/SR006.nfs2/dsuhoi/distill_direct_response_v12/eval_2020_8dpm/direct_gt_control.json
TEACHER_EVAL=/home/jovyan/shares/SR006.nfs2/dsuhoi/distill_shared_merge_v15/eval_2020_8dpm/merge_a050.json
CONTROL_EXP=exp_weatherdcae_14m_6h_sparse_gt_control_s202712_v17_bs4
STUDENT_EXP=exp_weatherdcae_14m_6h_sparse_flow_student_s202712_v17_bs4
...
if [[ "$PRECHECK_ONLY" -eq 1 ]]; then
  ...
  echo "[$(date -Is)] WeatherDCAE sparse-distillation precheck complete"
  exit 0
fi
...
train_one() {
  ...
  (
    ...
    "$PY" -u tools/train/train_capacity_matched_6h.py \
      ...
      --release_memmap_pages --precision bf16-mixed --seed 202712 \
...
if [[ ! -e "$EVAL/.complete" ]]; then
  ...
  (
    ...
    "$PY" -u tools/eval/batch_eval_12h_memmap.py \
      ...
      --models "dcae_gt_control:$CONTROL,weatherdcae_flow_student:$STUDENT" \
      --out-dir "$EVAL" --paper-tag weatherdcae_sparse_distill_v17_2020 \
...
(cd "$SOURCE" && "$PY" tools/eval/select_specialist_distillation.py \
  --control "$BASE_EVAL" \
  --candidate "weatherdcae_flow_student:$EVAL/weatherdcae_flow_student.json" \
  --aggregate-tolerance-pct 0.02 --held-tolerance-pct 0.02 \
  --out "$OUT_ROOT/selection.starting_base.json") || true
(cd "$SOURCE" && "$PY" tools/eval/select_specialist_distillation.py \
  --control "$EVAL/dcae_gt_control.json" \
  --candidate "weatherdcae_flow_student:$EVAL/weatherdcae_flow_student.json" \
  --aggregate-tolerance-pct 0.02 --held-tolerance-pct 0.02 \
...
if (cd "$SOURCE" && "$PY" tools/eval/select_distilled_champion.py \
  ...
  echo "[$(date -Is)] WeatherDCAE sparse student strict screen passed"
else
  touch "$STATE/strict.failed"
  echo "[$(date -Is)] WeatherDCAE sparse student strict screen failed"
  exit 2
fi
