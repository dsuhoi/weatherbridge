...
FROZEN_TRAINER="${FROZEN_TRAINER:-legacy/training_snapshots/train_capacity_matched_6h_32f4ce54.py}"
FROZEN_TRAINER_SHA256="${FROZEN_TRAINER_SHA256:-32f4ce54ddd3d033ea4ea55112d6e65658caf73961e568680aa627e8794e1f30}"
...
assessment="metrics/upr_lite_screen/upr_query_match_14m_pilot.json"
...
if [[ "$actual_trainer_sha256" != "$FROZEN_TRAINER_SHA256" || \
  ...
  echo "frozen QueryMatch trainer SHA-256 mismatch" >&2
  exit 2
fi

exec 9>"$LOG_ROOT/${exp_name}.queue.lock"
if ! flock -n 9; then
  echo "[$(date -Is)] QueryMatch pilot queue already active" \
    | tee -a "$queue_log"
  exit 0
fi
...
if candidate_ready \
  ...
  touch "$terminal_marker"
  echo "[$(date -Is)] reuse complete QueryMatch checkpoint=$checkpoint" \
    | tee -a "$queue_log"
  exit 0
fi
if [[ -e "$terminal_marker" && -s "$assessment" ]]; then
  ...
  if [[ "$promoted" -ne 1 ]]; then
    touch "$screened_marker"
    echo "[$(date -Is)] reuse excluded QueryMatch pilot" \
      | tee -a "$queue_log"
    exit 0
  fi
fi
...
run_stage() {
  ...
  echo "[$(date -Is)] launch QueryMatch epochs=8 gpu=$gpu" \
    | tee -a "$queue_log"
  (
...
while ! candidate_ready "$checkpoint" 2 3284; do
  ...
  if candidate_ready "$checkpoint" 2 3284; then
    echo "[$(date -Is)] complete QueryMatch two-epoch screen" \
      | tee -a "$queue_log"
    continue
  fi
...
echo "[$(date -Is)] promote QueryMatch to eight epochs" \
  | tee -a "$queue_log"
...
touch "$terminal_marker"
flock -u "$lock_fd"
echo "[$(date -Is)] complete QueryMatch checkpoint=$checkpoint" \
  | tee -a "$queue_log"
