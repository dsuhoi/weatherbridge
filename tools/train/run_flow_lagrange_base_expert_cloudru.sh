...
FROZEN_TRAINER="legacy/training_snapshots/train_capacity_matched_6h_cf4a8519.py"
FROZEN_TRAINER_SHA256="cf4a85190bac7bcdf14e5b8d49de49218645884fe6316f77e2cc73b830c1ae82"
...
declare -A EXP_NAMES=(
  [base_balanced]="exp_flow_compact_lagrange_l_baseft_9m_6h_s202707_v1"
  [base_edge_balanced]="exp_flow_compact_lagrange_l_baseedgeft_9m_6h_s202707_v1"
)

QUEUE_LOG="$LOG_ROOT/lagrange_base_expert.queue.log"
TERMINAL_MARKER="$LOG_ROOT/lagrange_base_expert.terminal"
COMPLETE_MARKER="$LOG_ROOT/lagrange_base_expert.complete"
...
acquire_gpu() {
  ...
  while true; do
    for candidate in $GPU_CANDIDATES; do
      local candidate_fd
      exec {candidate_fd}>"$LOG_ROOT/.lagrange_base_gpu${candidate}.lock"
...
if [[ "$PREFLIGHT_ONLY" == 1 ]]; then
  echo "[$(date -Is)] Lagrange base-expert preflight passed" \
    | tee -a "$QUEUE_LOG"
  trap - ERR
  exit 0
fi
...
trap - ERR
echo "[$(date -Is)] Lagrange base-expert fine-tunes complete" \
  | tee -a "$QUEUE_LOG"
