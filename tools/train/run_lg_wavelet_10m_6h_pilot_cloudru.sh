...
EXPECTED_PARAMETERS="9685168"

FROZEN_TRAINER="legacy/training_snapshots/train_capacity_matched_6h_b11ace95.py"
FROZEN_TRAINER_SHA256="b11ace95c7320f3ce87c0949b55d84d789d99ff3cbda93eb300362e7c765bca1"
MODEL_SOURCE="weather_time_interp/model/local_global_wavelet_bridge_model.py"
MODEL_SOURCE_SHA256="1094b424af4bc1cceb9710e6ce9bc19c7f3adea16c217f12a88ed498f35a7e82"
SPHERICAL_SOURCE="weather_time_interp/model/weatherbridge_flow_model.py"
SPHERICAL_SOURCE_SHA256="dc81b01feaa39c7a768c27549fa51a88f7bd38356f8d12b8fe9ee09bbaaeb185"
...
assert hparams["arch"] == "lg_wavelet_10m"
assert math.isclose(float(hparams["lambda_hf_override"]), 0.05)
assert math.isclose(float(hparams["lambda_spec_override"]), 0.0)
assert hparams["spectral_mask_profile"] == "all"
...
assert protocol["lambda_hf"] == 0.05
assert protocol["lambda_spec"] == 0.0
assert protocol["highpass_boundary"] == "antipodal_vector_parity"
...
assert net_parameters == int(sys.argv[4])
code = hparams["training_code_sha256"]
assert code["train_capacity_matched_6h.py"] == sys.argv[2]
assert code["local_global_wavelet_bridge_model.py"] == sys.argv[3]
assert code["weatherbridge_flow_model.py"] == sys.argv[5]
' "$checkpoint" "$FROZEN_TRAINER_SHA256" "$MODEL_SOURCE_SHA256" \
    "$EXPECTED_PARAMETERS" "$SPHERICAL_SOURCE_SHA256"
}
...
verify_source "$SPHERICAL_SOURCE" "$SPHERICAL_SOURCE_SHA256"
...
while ! candidate_ready; do
  ...
  echo "[$(date -Is)] launch arch=$ARCH gpu=$GPU attempt=$attempt" \
    | tee -a "$QUEUE_LOG"
  set +e
  (
  ...
  train_status=$?
  set -e
  echo "[$(date -Is)] finish train=$train_status monitor=$monitor_status" \
    | tee -a "$QUEUE_LOG"
  if ! candidate_ready; then
    sleep "$SLEEP_SEC"
  fi
...
touch "$COMPLETE_MARKER"
echo "[$(date -Is)] complete two-epoch screen arch=$ARCH" \
  | tee -a "$QUEUE_LOG"
