...
LOSS_PROFILE="${LOSS_PROFILE:?set LOSS_PROFILE to uniform or base_edge_balanced}"
...
case "$LOSS_PROFILE" in
  uniform)
    ...
    ;;
  base_edge_balanced)
    profile_tag="baseedge"
    ;;
  *)
    echo "unsupported LOSS_PROFILE=$LOSS_PROFILE" >&2
    exit 2
    ;;
esac
...
EXP_NAME="${EXP_NAME:-exp_weatherbridge_geo_msf_l_9m_6h_s202707_${profile_tag}_v1_bs8}"
...
net_parameters = sum(
    ...
    and key not in {"net.normalization_mean", "net.normalization_std"}
)
assert net_parameters == 8884205
assert hparams["training_code_sha256"]["train_capacity_matched_6h.py"] == sys.argv[3]
assert hparams["training_code_sha256"]["weatherbridge_flow_model.py"] == sys.argv[4]
' "$checkpoint" "$LOSS_PROFILE" "$FROZEN_TRAINER_SHA256" "$MODEL_SOURCE_SHA256"
}
...
wait_for_gpu
echo "[$(date -Is)] launch profile=$LOSS_PROFILE gpu=$GPU" | tee -a "$QUEUE_LOG"
set +e
(
...
touch "$COMPLETE_MARKER"
trap - ERR
echo "[$(date -Is)] complete profile=$LOSS_PROFILE" | tee -a "$QUEUE_LOG"
