...
ARCH="flow_pp3_nodiff"
EXP_NAME="exp_flow_pp3_nodiff_spectral_14m_6h_s202707_v2_struct_bs4"
LAMBDA_HF="0.05"
LAMBDA_SPEC="0.02"
SPECTRAL_MASK="advected"
HIGHPASS_BOUNDARY="periodic_lon_replicate_lat"

FROZEN_TRAINER="legacy/training_snapshots/train_capacity_matched_6h_5bab6fa2.py"
FROZEN_TRAINER_SHA256="5bab6fa2e0496e16c2e07621def23ce4446f862470ff03c2ea04e7c931de2162"
MODEL_SOURCE="weather_time_interp/model/weatherbridge_flow_model.py"
MODEL_SOURCE_SHA256="dc81b01feaa39c7a768c27549fa51a88f7bd38356f8d12b8fe9ee09bbaaeb185"
...
assert hparams["arch"] == "flow_pp3_nodiff"
assert math.isclose(float(hparams["lambda_hf_override"]), 0.05)
assert math.isclose(float(hparams["lambda_spec_override"]), 0.02)
assert hparams["spectral_mask_profile"] == "advected"
...
assert protocol["lambda_spec"] == 0.02
assert protocol["spectral_mask_profile"] == "advected"
...
assert net_parameters == 14245013
assert hparams["training_code_sha256"]["train_capacity_matched_6h.py"] == sys.argv[2]
assert hparams["training_code_sha256"]["weatherbridge_flow_model.py"] == sys.argv[3]
...
touch "$COMPLETE_MARKER"
trap - ERR
echo "[$(date -Is)] complete arch=$ARCH" | tee -a "$QUEUE_LOG"
