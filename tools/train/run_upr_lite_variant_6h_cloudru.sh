...
ARCH="${ARCH:?set ARCH to an UPR-Lite variant}"
case "$ARCH" in
  upr_lite|upr_lite_lap|upr_lite_column|upr_lite_continuous|\
  upr_lite_continuous_m|upr_lite_implicit_global|\
  upr_lite_implicit_global_q4) ;;
  *)
    echo "unsupported UPR-Lite architecture: $ARCH" >&2
    exit 2
    ;;
esac
...
EXP_NAME="${EXP_NAME:-exp_${ARCH}_24ch_6h_2014_19_sparse135_lr1e4_b8_s202707}"
...
if ! flock -n 9; then
  echo "[$(date -Is)] queue already active; wait exp=$EXP_NAME" \
    | tee -a "$QUEUE_LOG"
  while ! "$PYTHON_BIN" tools/train/checkpoint_status.py \
    "$CHECKPOINT" --min-epochs 8 --quiet; do
    sleep "$SLEEP_SEC"
  done
  echo "[$(date -Is)] active queue completed checkpoint=$CHECKPOINT" \
    | tee -a "$QUEUE_LOG"
  exit 0
fi
...
while ! "$PYTHON_BIN" tools/train/checkpoint_status.py \
  ...
  for gpu in $GPU_CANDIDATES; do
    ...
    echo "[$(date -Is)] launch arch=$ARCH gpu=$gpu bs=$batch_size accumulate=$accumulate" \
      | tee -a "$QUEUE_LOG"
    ...
    flock -u "$gpu_fd"
    exec {gpu_fd}>&-
    echo "[$(date -Is)] finish arch=$ARCH status=$status" | tee -a "$QUEUE_LOG"
    ...
    if [[ "$status" -ne 0 ]] && \
      ...
      batch_size=4
      accumulate=4
      echo "[$(date -Is)] OOM fallback bs=4 accumulate=4" \
        | tee -a "$QUEUE_LOG"
    fi
