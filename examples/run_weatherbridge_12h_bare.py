"""WeatherBridge 14M (12h horizon) bare inference."""
...
def main():
    ...
    model = load_bare(
        REPO_ROOT
        / "weights"
        / "weatherbridge_14m_12h_bare.pt",
        device,
    )
    print(
        f"loaded WeatherBridge 12h: "
    ...
    static = torch.load(
        str(REPO_ROOT / "data" / "static_features_0p5.pt"), weights_only=False
    ).float()
    if static.dim() == 3:
        static = static.unsqueeze(0)
    static = static[:, : model.n_static_features].to(device)

    # 12 h window, predict mid τ=6 h.
    with torch.no_grad():
        out = model(
            x0,
            xT,
            torch.tensor([[6.0 / 12.0]], device=device),
            torch.tensor([12.0], device=device),
            static=static,
        )
