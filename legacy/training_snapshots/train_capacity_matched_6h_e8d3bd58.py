...
def build_net(arch: str, static_path: str):
    ...
    if arch in (
        ...
        net = WeatherBridgeModel(
            ...
            spectral_branch=(
                arch in ("flow_pp3", "flow_spherical_ep")
            ),
            hydro_couple=(
                arch in ("flow_pp3", "flow_spherical_ep")
            ),
            dual_stream=(arch == "flow_dual"),
            spherical_ops=(arch == "flow_spherical_ep"),
            endpoint_preserving=(arch == "flow_spherical_ep"),
...
def main():
    ...
    p.add_argument("--arch", required=True, choices=["weatherbridge", "weatherdcae", "pixelattn_vfi", "weatheramt", "dcae_14m", "crossframe", "atmvfi", "amt", "wb_vanilla", "wb_skip", "mamba",
    ...
    training_sources = {
        ...
        "training_protocol.py": Path(__file__).with_name(
