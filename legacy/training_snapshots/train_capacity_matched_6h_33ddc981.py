...
class ATMVFIStaticNet(nn.Module):
    """ATM-VFI (PixelAttentionVFINet) with static-augmented encoder.
...
def build_net(arch: str, static_path: str):
    ...
    if arch in (
        ...
        return net, False, True
    if arch == "upr_implicit_global_14m":
        from weather_time_interp.model.weatherbridge_upr_lite_model import (
            WeatherBridgeUPRLiteModel,
        )
        ...
        return net, False, True
    if arch in ("flow", "flow_noskip", "flow_ungated", "flow_accel", "flow_pp",
                "flow_pp2", "flow_pp3", "flow_dual"):
        ...
        net = WeatherBridgeModel(
            ...
            spectral_branch=(arch == "flow_pp3"),
            hydro_couple=(arch == "flow_pp3"),
...
class CapMatchedLit(pl.LightningModule):
    ...
    def _step(self, batch, prefix):
        ...
        err = ((pred - tgt).abs() * self.lat_w).mean()
        ...
        if prefix == "train" and self.lambda_hf > 0:
            pred_pad = F.pad(pred, (1, 1, 0, 0), mode="circular")
            pred_pad = F.pad(pred_pad, (0, 0, 1, 1), mode="replicate")
            tgt_pad = F.pad(tgt, (1, 1, 0, 0), mode="circular")
            tgt_pad = F.pad(tgt_pad, (0, 0, 1, 1), mode="replicate")
            pred_hf = pred - F.avg_pool2d(pred_pad, kernel_size=3, stride=1)
            tgt_hf = tgt - F.avg_pool2d(tgt_pad, kernel_size=3, stride=1)
            hf = ((pred_hf - tgt_hf).abs() * self.lat_w).mean()
...
def main():
    ...
    p.add_argument("--arch", required=True, choices=["weatherbridge", "weatherdcae", "dcae_14m", "crossframe", "atmvfi", "wb_vanilla", "wb_skip", "mamba",
    ...
    steps_per_epoch = max(1, len(train_ds) // (args.bs * args.accumulate * max(1, len(args.gpus))))
    ...
    trainer_train_batch_limit: float | int = args.limit_train_batches
    if args.train_batches_per_epoch:
        if args.train_batches_per_epoch < 1:
            raise ValueError("train_batches_per_epoch must be positive")
        if args.train_batches_per_epoch > len(train_loader):
            raise ValueError(
                "train_batches_per_epoch exceeds available train batches: "
                f"{args.train_batches_per_epoch} > {len(train_loader)}"
            )
        trainer_train_batch_limit = args.train_batches_per_epoch

    model = CapMatchedLit(
        arch=args.arch, static_path=args.static_path, lr=args.lr,
        delta_t=float(args.window_hours), eval_taus=tuple(args.eval_tau),
        total_steps=total_steps, training_seed=args.seed,
    ...
    model.hparams["training_protocol"] = {
        ...
        "lambda_hf_override": args.lambda_hf_override,
        "train_batches_per_epoch": args.train_batches_per_epoch or None,
    }
    training_sources = {
    ...
    if args.arch == "upr_implicit_global_14m":
    ...
    print(f"  params: {n_p:.2f}M  total_steps={total_steps}", flush=True)
    if args.init_only:
