...
def build_net(arch: str, static_path: str):
    ...
    if arch in (
        ...
        _hidden = (
            32
            if arch == "flow_compact_vp3"
            else 64 if arch == "flow_dual" else 72
        )
        net = WeatherBridgeModel(
            ...
            spherical_ops=(
                arch in ("flow_spherical_ep", "flow_compact_vp3")
            ),
            endpoint_preserving=(
                arch in ("flow_spherical_ep", "flow_compact_vp3")
            ),
            ...
            n_flow_modes=3 if arch == "flow_compact_vp3" else 1,
            cubic_trajectory=(arch == "flow_compact_vp3"),
            global_tokens=8 if arch == "flow_compact_vp3" else 0,
            global_token_dim=32,
        )
        return net, False, True
