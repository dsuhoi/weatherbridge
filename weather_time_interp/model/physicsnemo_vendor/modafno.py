...
from ._shim import ModelMetaData, Module

# Import AFNO layers from local vendor copy (mirrors physicsnemo.nn upstream)
...
class ModAFNO(Module):
    ...
    def __init__(
        ...
        # ModEmbedNet must run unconditionally because mod_additive_proj always
        # consumes its (B, mod_dim) output; the original `nn.Identity` fallback
        # was a bug — left tau as (B, 1) and broke the additive projection when
        # both modulate_mlp and modulate_filter were False.
