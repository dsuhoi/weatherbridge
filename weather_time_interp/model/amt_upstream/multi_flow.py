...
from .flow_utils import SphereConvTranspose2d, warp
from .ifrnet import (
    ResBlock,
    convrelu,
    match_spatial_size,
...
def multi_flow_combine(
    ...
    warp_fn=warp,
        ...
        '''
        b, flow_channels, h, w = flow0.shape
        field_channels = img0.shape[1]
        num_flows = flow_channels // 2
        ...
        img_res = img_res.reshape(b, num_flows, field_channels, h, w
                            ).reshape(-1, field_channels, h, w) if img_res is not None else 0
        img0 = torch.stack([img0] * num_flows, 1).reshape(
            -1, field_channels, h, w
        )
        img1 = torch.stack([img1] * num_flows, 1).reshape(
            -1, field_channels, h, w
        )
        mean = torch.stack([mean] * num_flows, 1).reshape(
            -1, field_channels, 1, 1
                                                    ) if mean is not None else 0
        
        img0_warp = warp_fn(img0, flow0)
        img1_warp = warp_fn(img1, flow1)
        img_warps = mask * img0_warp + (1 - mask) * img1_warp + mean + img_res
        img_warps = img_warps.reshape(
            b, num_flows, field_channels, h, w
        )
...
class MultiFlowDecoder(nn.Module):
    ...
    def forward(
        ...
        delta_flow0, delta_flow1, mask, img_res = torch.split(
            out,
            [2 * n, 2 * n, n, self.field_channels * n],
            1,
        )
