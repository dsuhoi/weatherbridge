...
def bilinear_sampler(img, coords, mask=False):
    """Sample a correlation image in spherical pixel coordinates."""
    sampled = spherical_grid_sample(img, coords)

    if mask:
        valid = torch.isfinite(coords).all(dim=-1, keepdim=True)
...
class SmallUpdateBlock(nn.Module):
    def __init__(self, cdim, hidden_dim, flow_dim, corr_dim, fc_dim,
        ...
        self.convf1 = SphereConv2d(4, flow_dim*2, 7, padding=3)
        self.convf2 = SphereConv2d(flow_dim*2, flow_dim, 3, padding=1)
        self.conv = SphereConv2d(corr_dim+flow_dim, fc_dim, 3, padding=1)

        self.gru = nn.Sequential(
            SphereConv2d(fc_dim+4+cdim, hidden_dim, 3, padding=1),
            nn.LeakyReLU(negative_slope=0.1, inplace=True),
            SphereConv2d(hidden_dim, hidden_dim, 3, padding=1),
        )

        self.feat_head = nn.Sequential(
            SphereConv2d(hidden_dim, hidden_dim, 3, padding=1),
            nn.LeakyReLU(negative_slope=0.1, inplace=True),
            SphereConv2d(hidden_dim, cdim, 3, padding=1),
        )

        self.flow_head = nn.Sequential(
            SphereConv2d(hidden_dim, hidden_dim, 3, padding=1),
            nn.LeakyReLU(negative_slope=0.1, inplace=True),
            SphereConv2d(hidden_dim, 4, 3, padding=1),
        )
...
class BasicUpdateBlock(nn.Module):
    def __init__(self, cdim, hidden_dim, flow_dim, corr_dim, corr_dim2, 
        ...
        self.scale_factor = scale_factor
        self.convc1 = nn.Conv2d(2 * cor_planes, corr_dim, 1, padding=0)
        self.convc2 = SphereConv2d(corr_dim, corr_dim2, 3, padding=1)
        self.convf1 = SphereConv2d(4, flow_dim*2, 7, padding=3)
        self.convf2 = SphereConv2d(flow_dim*2, flow_dim, 3, padding=1)
        self.conv = SphereConv2d(flow_dim+corr_dim2, fc_dim, 3, padding=1)
        ...
        self.flow_head = nn.Sequential(
            SphereConv2d(hidden_dim, hidden_dim, 3, padding=1),
            nn.LeakyReLU(negative_slope=0.1, inplace=True),
            SphereConv2d(hidden_dim, 4*out_num, 3, padding=1),
        )
...
class BidirCorrBlock:
    def __init__(self, fmap1, fmap2, num_levels=4, radius=4):
        ...
        for _ in range(self.num_levels-1):
            corr = spherical_avg_pool2d(corr)
            corr_T = spherical_avg_pool2d(corr_T)
            self.corr_pyramid.append(corr)
            self.corr_pyramid_T.append(corr_T)
