#!/usr/bin/env python3
"""DC-AE autoencoder f=32 spatial × latent_ch=512 on 3yr fibo data.

Less aggressive overall compression (54×) than f=32+64ch (456×) by widening
the latent channel axis. Keeps small spatial 10×24 grid (DiT-friendly) but
gives decoder 8× more bandwidth per spatial cell.

Architecture: 6 stages (192,192,384,384,512,512), 12 layers, EfficientViT
on stages 5-6, latent_ch=512. ~95M params.
"""
...
class WeatherDCAEAutoencoder(pl.LightningModule):
    """f=32 spatial, latent_ch=512. 54× compression with wider channel latent."""

    def __init__(self,
                 in_channels=27,
                 latent_channels=512,
                 block_out_channels=(192, 192, 384, 384, 512, 512),
                 layers_per_block=(2, 2, 2, 2, 2, 2),
                 block_type=('ResBlock', 'ResBlock', 'ResBlock', 'ResBlock',
                             'EfficientViTBlock', 'EfficientViTBlock'),
                 qkv_multiscales=((), (), (), (), (5,), (5,)),
                 attention_head_dim=32,
                 lat_crop=40, lon_pad=48,
...
def main():
    ...
    p.add_argument('--latent_channels', type=int, default=512)
    p.add_argument('--exp_name', default='exp_dcae_ae_f32_lat512_3yr_fibo')
