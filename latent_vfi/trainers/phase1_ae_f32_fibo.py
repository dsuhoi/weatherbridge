#!/usr/bin/env python3
"""DC-AE autoencoder (f=32 spatial compression) on 3yr fibo data.

For TLB-VFI-comparable latent compression ratio. Smaller spatial latent
(64×10×24 = ~15k floats → 450× compression vs unfolded 27×360×720).

Note: lat_crop=40 (drops 10° each pole) + lon_pad=48 needed for div-by-64
geometry through 5 downsamples in SphereConv. This is a paper-grade ablation
of "more aggressive compression" — losing polar coverage is documented as limitation.

Run inside container with /tmp/wb2_0p5_cache containing 2017-2020 data.
Training years: 2017-2019 (3yr), val: 2020.
"""
...
class WeatherDCAEAutoencoderF32(pl.LightningModule):
    """f=32 spatial, 6 stages, latent 64×10×24."""

    def __init__(self,
                 in_channels=27,
                 latent_channels=64,
                 # Smaller channels to keep params reasonable (~120M)
                 ...
                 lat_crop=40,                 # 360-40=320, 320/32=10
                 lon_pad=48,                  # 720+48=768, 768/32=24
                 sst_idx=SST_IDX,
                 sst_sentinel=0.0,
...
def main():
    ...
    p.add_argument('--exp_name', default='exp_dcae_ae_f32_3yr_fibo')
    ...
    model = WeatherDCAEAutoencoderF32(
