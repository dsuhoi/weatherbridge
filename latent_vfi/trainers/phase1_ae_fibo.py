#!/usr/bin/env python3
"""DC-AE autoencoder f=16 spatial × latent_ch=128 on 3yr fibo data.

Same architecture width as cloud.ru AE but latent_ch doubled (64→128).
Spatial: 22×46 (vs cloud.ru's same 22×46 with lat=64). Total compression 54×
(vs cloud.ru 108×).

Architecture: 5 stages (192,192,384,384,768), layers (3,3,3,3,3),
EfficientViT on last 2 stages, latent_ch=128. ~124M params.
Latent: 128 × 22 × 46 ≈ 130k floats → 54× compression.

vs prior f=32 lat=512 (also 54×): same total compression but 4× more spatial
granularity (22×46 = 1012 cells vs 10×24 = 240) — better for mesoscale features.
"""
...
class WeatherDCAEAutoencoder(pl.LightningModule):
    """f=16 spatial, latent_ch=128. 54× compression, 1012 spatial cells."""
    ...
    def __init__(self,
                 ...
                 lat_crop=8, lon_pad=16,
                 sst_idx=SST_IDX, sst_sentinel=0.0,
                 static_features_path='data/static_features_0p5.pt',
                 n_static=3,
                 lr=2e-4, weight_decay=1e-4, betas=(0.9, 0.95)):
        super().__init__()
        self.save_hyperparameters()
        self.lat_crop = lat_crop; self.lon_pad = lon_pad
        self.sst_idx = sst_idx; self.sst_sentinel = sst_sentinel
        self.n_static = n_static

        self.encoder = DCAEEncoder(
            in_channels=in_channels, latent_channels=latent_channels,
            block_type=block_type, block_out_channels=block_out_channels,
            layers_per_block=layers_per_block, qkv_multiscales=qkv_multiscales,
            attention_head_dim=attention_head_dim,
            downsample_block_type='pixel_unshuffle', out_shortcut=True)
        self.decoder = DCAEDecoder(
            out_channels=in_channels, latent_channels=latent_channels,
            block_type=block_type, block_out_channels=block_out_channels,
            layers_per_block=layers_per_block, qkv_multiscales=qkv_multiscales,
            attention_head_dim=attention_head_dim,
            upsample_block_type='pixel_shuffle', in_shortcut=True)
        self.static_proj = nn.Conv2d(latent_channels + n_static, latent_channels,
                                     kernel_size=1)
        with torch.no_grad():
            self.static_proj.weight.zero_(); self.static_proj.bias.zero_()
    ...
    def _crop_lat(self, x):
        top = self.lat_crop // 2; bot = self.lat_crop - top
        return x[:, :, top:-bot] if bot > 0 else x[:, :, top:]

    def _pad_lon(self, x):
        l = self.lon_pad // 2; r = self.lon_pad - l
    ...
    def forward(self, x):
        x_c = self._crop_lat(x); x_c = self._pad_lon(x_c)
        z = self.encoder(x_c)
        static_lat = F.adaptive_avg_pool2d(self.static_full, z.shape[-2:])
    ...
    def _apply_sst_mask(self, t):
        ...
        out = t.clone(); out[:, self.sst_idx] = sst
        return out
    ...
    def on_validation_epoch_end(self):
        ...
        if self.val_count.item() > 0:
            ...
            self.log('val_rmse/surface_mean',
                     ...
                     rank_zero_only=True)
        self.val_sq_sum.zero_(); self.val_count.zero_()
    ...
    def configure_optimizers(self):
        ...
        warmup_steps = min(500, total_steps // 30)

        def lr_lambda(step):
            if step < warmup_steps:
                return step / max(1, warmup_steps)
            progress = (step - warmup_steps) / max(1, total_steps - warmup_steps)
            # Cosine decay to 10% of max lr (not zero, for late refinement)
...
def main():
    ...
    p.add_argument('--exp_name', default='exp_dcae_ae_f16_lat128_3yr_fibo')
    # NOTE: lat=64 ablation expected to use --latent_channels 64 --exp_name exp_dcae_ae_f16_lat64_3yr_fibo
    ...
    out_dir = Path('/workspace/code/wti/logs') / args.exp_name
    ...
    if len(args.gpus) > 1:
        ...
        print(f'DDP strategy: backend={backend}')
    else:
        strategy = 'auto'
    trainer = pl.Trainer(
