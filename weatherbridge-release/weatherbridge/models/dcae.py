...
# These are only required by the top-level AutoencoderDC class (not used by
# Encoder/Decoder which we wrap directly). Wrap them so cluster envs with broken
# diffusers.loaders/transformers integration can still import this module.
try:
    ...
    _AUTOENCODER_DC_AVAILABLE = True
except (ImportError, RuntimeError):
    _AUTOENCODER_DC_AVAILABLE = False
    # Use distinct base classes to avoid "duplicate base class object" error.
    class _DummyConfigMixin: pass
    class _DummyModelMixin: pass
    class _DummyFromOriginalModelMixin: pass
    ConfigMixin = _DummyConfigMixin
    ModelMixin = _DummyModelMixin
    FromOriginalModelMixin = _DummyFromOriginalModelMixin
    DecoderOutput = EncoderOutput = None
